/*
 * PyTorch extension for CPU backend
...
static std::unique_ptr<CPURasterizer> g_cpu_rasterizer;
...
RasterizetrianglesCUDA(
    ...
    if (!g_cpu_rasterizer) {
        g_cpu_rasterizer = std::make_unique<CPURasterizer>();
    }
    ...
    // Call CPU rasterizer
    int result = g_cpu_rasterizer->rasterize_triangles(
        args,
    ...
    if (result != 0) {
        throw std::runtime_error("CPU rasterization failed");
    }
...
RasterizetrianglesBackwardCUDA(
    ...
    if (!g_cpu_rasterizer) {
        throw std::runtime_error("CPU rasterizer not initialized");
    }
    ...
    // Call CPU backward pass
    int result = g_cpu_rasterizer->rasterize_triangles_backward(
        args,
    ...
    if (result != 0) {
        throw std::runtime_error("CPU backward pass failed");
    }
...
torch::Tensor markVisible(
    ...
    // Simple visibility marking for CPU backend
    const int P = means3D.size(0);
...
torch::Tensor ComputeRelocationCUDA(
    ...
    // Simple relocation computation for CPU backend
    const int P = means3D.size(0);
